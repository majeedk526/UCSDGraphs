...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		...
		List<Integer> scndNeighbors = new ArrayList<Integer>();
		List<Integer> firstNeighbors = getNeighbors(v);		
		
		for(int i=0; i<firstNeighbors.size(); i++){
				int tmpv = firstNeighbors.get(i);
				List<Integer> neighbors = getNeighbors(tmpv);
				
				for(int j=0; j<neighbors.size(); j++){
					scndNeighbors.add(neighbors.get(j));
				}
		}
		
		Collections.sort(scndNeighbors);
		
		return scndNeighbors;
	}
